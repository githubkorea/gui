...
public class CardLayout2 extends JFrame implements ActionListener{
	...
	public CardLayout2( ) {
		...
		//center에 보여줄 레이아웃 담당 - 카드 레리아웃 설정
		panel_2 = new JPanel();
	...
	public void actionPerformed(ActionEvent e) {
		...
		JPanel p = (JPanel)getContentPane().getComponent(1);  //  contentpane은 JPanel 로 선언했으니 JPanel로 받고
		CardLayout card = (CardLayout) p.getLayout(); 		// layout은 cardLayout 으로 받았으니 cardlayout으로 받고 
		
		if(btn==mainBtn1) { 	// 버튼을 눌렀을 때 원하는 패널이 뜨도록  0번은 top NORTH에 붙은 그놈임 .
			card.show(p, "1"); 	//원하는 패널을 뽑기 위해선 번호가 필요해 그래서 위에서 정해줌
